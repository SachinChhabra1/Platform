...
| Test coverage breadth | 🟡 | screens/components/formatPaise covered; `widgets/common.dart` has no direct test; backend services not covered here (paused) |
...
2. **`widgets/common.dart` test gap** — `Monogram`, `prototypeNoOp`, `openNiaEmergency` have no
   direct test. In-authority, golden-neutral to close. **Candidate next slice.**
3. **No CI** — tests run locally only. Adding CI is in-authority (DX/reliability) but bigger; record
   as a roadmap item rather than a snap change.
