...
| Infra | Integration hardening | 🔓 DONE (this round) | Service auth (`SecretServiceAuthenticator`); remittance rail webhooks (`openapi.rail.yaml`); scheduled SLA sweep + savings accrual/settlement jobs on a service-authed ops surface (`openapi.ops.yaml`); read-only Operator reconciliation surface; durable-store primitive (`FileDurableStore`, survives restart) + durable `RemittanceStore`; production config loader + `composeWalletApp` (every Founder value read from env, none invented). `services/wallet` → 214 tests. **Opened OD-8** (Operator money-conflict resolution) per Step-5 — read-only surface shipped, resolve action gated. Remaining (no OD): durable adapters for the other ports + the Postgres adapter (ADR-0006, online); RafiQi auto-take/compensation. |
