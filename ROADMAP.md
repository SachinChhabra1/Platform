...
| R5 | RafiQi orchestration (backend) | 🔓 IN PROGRESS | **OD-3 ruled** ([ADR-0014](docs/adr/0014-rafiqi-reversibility-and-consent.md), Locked). Domain done: scoped/capped/time-bounded/revocable standing authorisation, `authorizeAction` (auto vs per-action-confirmation fallback), 24h reversibility (`takeAction`/`reverseAction`/`checkReversibility`) + grant/action stores, +13 tests (`rafiqi.ts`). Next: HTTP endpoints + `openapi.rafiqi.yaml`, wire reversal to the target money path. |
