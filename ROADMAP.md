...
| R4 | Remittance completion (backend) | 🔓 IN PROGRESS | **OD-2 ruled** ([ADR-0013](docs/adr/0013-remittance-confirmed-and-sla.md), Locked). Domain done: state machine ("sent" ≠ confirmed; confirmed = recipient-available; 24h SLA → Operator; family ack optional) + escalation/store seams, +12 tests (`remittance.ts`). Next: HTTP endpoint + `openapi.remittance.yaml`, rail adapter, SLA sweep. |
