...
| R3 | Wage Flow slice (backend) | 🔓 DONE | **OD-1 ruled** ([ADR-0012](docs/adr/0012-wage-flow-shortfall-priority.md), Locked) + **OD-7 ruled** ([ADR-0018](docs/adr/0018-arrears-recovery-ordering.md), Locked). Allocator + `POST /v1/wage/settlements` (server-side `FloorSource`), arrears carry-forward + distinct waiver recording, **and arrears recovery** (`planArrearsRecovery`): current-cycle waterfall first, then surplus-above-floor only, oldest-first, capped (Founder config, 50%), Nia's fee/advance last; take-home net of recovery; money conserved. Both halves of R3 complete. Cap is injected config, never baked in. |
...
| R8 | Dignity gates / The Floor | 🔓 DONE (domain+endpoint) | **OD-6 ruled** ([ADR-0017](docs/adr/0017-the-floor-authoritative-source.md), Locked). The versioned, Founder-owned, audited `the_floor` config (`the_floor.ts`): append-only version history **is** the change-audit trail (`createInitialFloor`/`reviseFloor`, monotonic, never rewritten); contents per ADR (baseline dignity floor, settlement floor, FD-11 women-Member higher floor, server-side per-Member overrides). Server-side accessor `RegistryFloorSource` implements the existing `FloorSource` seam — **plugged into wage settlement**, proven by test (take-home tracks the registry floor and a revision moves it, server-side). Read-only `GET /v1/floor` (`openapi.floor.yaml`) returns only the public guarantees (per-Member overrides never leak), default-deny, no mutation route. **No Floor value invented — Founder-provided / fixtures only** (+17 tests). Now wired into the boot via `composeWalletApp` (seeded from `NIA_FLOOR_CONFIG_PATH`). |
| Infra | Integration hardening | 🔓 DONE | Service auth (`SecretServiceAuthenticator`); remittance rail webhooks (`openapi.rail.yaml`); scheduled SLA sweep + savings accrual/settlement jobs on a service-authed ops surface (`openapi.ops.yaml`); Operator reconciliation surface — **read + resolve**; production config loader + `composeWalletApp` (every Founder value read from env, none invented). |
| Infra | Durable persistence + Postgres seam + RafiQi orchestrator | 🔓 DONE | `DurableStore<T>` primitive (`FileDurableStore`, survives restart) + **durable adapters for every wallet port** (compose is fully persistent); **PostgreSQL seam** (`PostgresDurableStore` over an `SqlExecutor` port — no `pg` dependency; `pg.Pool` satisfies it online); **RafiQi orchestrator** (auto-take under an active grant + 24h reversal compensation via an injected `MoneyEffect` seam, ADR-0014). `services/wallet` → 245 tests. Remaining is online-only (real `pg` + a database) or Founder-gated (concrete config values, concrete money paths behind the seams). |
| OD-8 | Operator conflict resolution | 🔓 DONE | **OD-8 ruled** (the ADR-0015 resolution gap; [ADR-0019](docs/adr/0019-operator-conflict-resolution.md), Locked). `resolveConflict` (accept-proposal / keep-server / manual — each an authoritative ledger write recording operator + reason) + per-operator identity (`X-Nia-Operator-Token`) + `POST /v1/ops/reconciliation/{id}/resolve`. Credentials are ops-owned config (empty ⇒ deny). Completes the write half of the reconciliation surface. |
