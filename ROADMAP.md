...
| Infra | Integration hardening | 🔓 DONE | Service auth (`SecretServiceAuthenticator`); remittance rail webhooks (`openapi.rail.yaml`); scheduled SLA sweep + savings accrual/settlement jobs on a service-authed ops surface (`openapi.ops.yaml`); Operator reconciliation surface — **read + resolve**; durable-store primitive (`FileDurableStore`, survives restart) + durable `RemittanceStore`; production config loader + `composeWalletApp` (every Founder value read from env, none invented). `services/wallet` → 224 tests. Remaining (no OD): durable adapters for the other ports + the Postgres adapter (ADR-0006, online); RafiQi auto-take/compensation. |
